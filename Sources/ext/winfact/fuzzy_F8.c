...
static int membership(const FuzzySetF8_t *s, NumTypeF8_t e, NumTypeF8_t *mv)
{
  register const NumTypeF8Point_t* p=s->p;
  register NumOfVal_t n=s->n;

  if ( opF8L(e, p[0].x) || opF8G(e, p[n-1].x) ) return 0;
  if (n==1){ /*singleton*/
    if (opF8E(e, p[0].x)){
      *mv=1;
      return 1;
    ...
    for (i=1; i<s->n; i++)
      if (opF8LE(e, s->p[i].x)){
    	if (opF8NE(p[i].x, p[i-1].x)){
	  *mv=opF8Add(
		    opF8Div(
			  opF8Mul(
				opF8Sub(p[i].y,  p[i-1].y),
				opF8Sub(e,  p[i-1].x)
				),
			  opF8Sub(p[i].x,  p[i-1].x)
			  ),
		    p[i-1].y
		    );
        }else {
          if (opF8G(p[i].y, p[i-1].y)) *mv=p[i].y;
	  else *mv=p[i-1].y;
        }
        return *mv>0;
      }
...
static NumTypeF8_t FuzzyAnd(const FuzzyControllerF8_t *fc, const NumTypeF8_t mue1, const NumTypeF8_t mue2)
{
  NumTypeF8_t zw;
  switch (fc->AndOp){
  case AND_BOUNDED_DIFF:
    zw=opF8Sub(opF8Add(mue1,mue2),F8One);
    return opF8G(zw,0) ? zw : 0;
  case AND_ALGEBRAIC_SUM:
    return opF8Mul(mue1,mue2);
  default: /* AND_MIN*/
    return (opF8G(mue1,mue2))? mue2 : mue1;
  }
}


static NumTypeF8_t FuzzyInference(const FuzzyControllerF8_t *fc,  const NumTypeF8_t mue, const NumTypeF8_t muex)
{
  switch (fc->inf){ 
  case  MAX_PROD :
    return opF8Mul(muex, mue);
  default: /*MAX_MIN*/   
    return opF8G(muex, mue) ? mue : muex;
  }
}


static void AproxCenterOfGravity(const FuzzyControllerF8_t *fc,  FCMemF8_t *v)
{
  const FuzzySetF8_t *s;
  NumTypeF8_t    x;
  NumTypeF8_t    n, z;
  NumOfVal_t  i, j;
  NumTypeF8_t   xmin, xdiff;
  ...
  for (i=0; i<fc->nO; i++){
    ...
    xdiff=opF8Sub(s->p[s->n-1].x,xmin);
    for (j=0; j<fc->oL[i].n; j++){
      ...
      case 4 /*Trapezoid*/  : x=opF8Div(opF8Add(s->p[1].x, s->p[2].x), opF8Cast(2));
      }
      /* scaling */
      x=opF8Div(opF8Sub(x,xmin),xdiff);
      /* end scaling */
      z=opF8Add(z, opF8Mul(v->omv[i][j], x));
      n=opF8Add(n, v->omv[i][j]);
    }
    if (n) v->res[i]=opF8Add(opF8Mul(opF8Div(z, n),xdiff),xmin);
...
static void ModifiedCenterOfGravity(const FuzzyControllerF8_t *fc,  FCMemF8_t *v)
{
  const FuzzySetF8_t *s0, *sn, *sb, *se, *s;
  NumTypeF8_t      dx;
  NumTypeF8_t      x;
  NumTypeF8_t      mv, mv0;
  NumTypeF8_t      mvx;
  NumTypeF8_t     n, z;
  NumTypeF8_t     n0, z0, nn, zn;
  NumOfVal_t    i, k;
  NumTypeF8_t     xmin, xdiff;
  ...
  for (i=0; i<fc->nO; i++){
    ...
    dx=opF8Div(
	     opF8Sub(se->p[se->n-1].x, sb->p[0].x),
	     opF8Cast(fc->steps)
	     );
    xmin=sb->p[0].x;
    xdiff=opF8Sub(se->p[se->n-1].x,xmin);
    se++;   /*points behind the last active set*/
    for (k=0; k<=fc->steps; k++){
      x=opF8Add(
	      opF8Mul( opF8Cast(k),	dx),
	      sb->p[0].x
	      );
      mv=0;
      for (s=sb; s!=se; s++){
	...
	if (opF8L(mv, mv0)) mv=mv0;
	  ...
	  x0=opF8Sub(
		       opF8Add(s->p[0].x, 
				 s->p[0].x), 
		       x);
          if (opF8NE(x0, x)){  /*to be sure that no point is taken twice*/
	    mv0=FuzzyInference(fc,v->omv[i][s-s0],mvx);
	    /*scaling*/
	    x0=opF8Div(opF8Sub(x0, xmin),xdiff); 
	    /*end scaling*/
            z0=opF8Add(
			 opF8Mul(x0, mv0),
			 z0
	  ...
	  xn=opF8Sub(
	    ...
	    mv0=FuzzyInference(fc,v->omv[i][s-s0],mvx);
	    /*scaling*/
	    xn=opF8Div(opF8Sub(xn, xmin),xdiff); 
	    /*end scaling*/
	    zn=opF8Add(
			 opF8Mul(xn, mv0),
			 zn
    ...
    z=opF8Add(opF8Add(z, z0), zn);
    n=opF8Add(opF8Add(n, n0), nn);
    if (n) v->res[i]=opF8Add(opF8Mul(opF8Div(z, n),xdiff),xmin);
  }
}
...
static void CenterOfGravity(const FuzzyControllerF8_t *fc,  FCMemF8_t *v)
{
  const FuzzySetF8_t *sb, *se, *s, *s0;
  NumTypeF8_t      dx;
  NumTypeF8_t      x;
  NumTypeF8_t      xmin, xdiff;
  NumTypeF8_t      mv;
...
static void maxheight(const FuzzyControllerF8_t *fc,  FCMemF8_t *v)
{
  const FuzzySetF8_t *s = NULL, *s0 = NULL;
  NumTypeF8_t      mv;
  NumOfVal_t    i, j;
  ...
  for (i=0; i<fc->nO; i++){
    ...
    for (j=0; j<fc->oL[i].n; j++){
      s=&fc->oL[i].fs[j];              /*s points to actual set now*/
      if (!v->ohit[i][j]) continue;
      if (((fc->method == DEFUZZY_MHR) && opF8LE(mv,v->omv[i][j])) ||  /*most right set with highest membership value*/
           opF8L(mv,v->omv[i][j]) ) {             /*most left set with highest membership value*/
        s0=s;
        mv=v->omv[i][j];
      }
    }
    ...
    case 4 /*Trapezoid*/  : v->res[i]=opF8Div(opF8Add(s0->p[1].x,s0->p[2].x),opF8Cast(2));
    }
  }
}


static void firstlastofmax(const FuzzyControllerF8_t *fc,  FCMemF8_t *v)
{
  const FuzzySetF8_t *s0;
  NumOfVal_t       i, j, k;
  char                no;
  ...
  for (i=0; i<fc->nO; i++){
    ...
    if (opF8Sub(s0->p[k].y, s0->p[k-1].y))
      v->res[i]=opF8Add(
					opF8Div(
							opF8Mul(
									opF8Sub(v->omv[i][no], s0->p[k-1].y),
									opF8Sub(s0->p[k].x, s0->p[k-1].x)
									),
							opF8Sub(s0->p[k].y, 
									s0->p[k-1].y)
					),s0->p[k-1].x);
...
//		}
		...
		v->imv  = (NumTypeF8_t**)calloc(sizeof(NumTypeF8_t*), fc->nI);
		v->omv  = (NumTypeF8_t**)calloc(sizeof(NumTypeF8_t*), fc->nO);
		v->res  = (NumTypeF8_t*)calloc(sizeof(NumTypeF8_t), fc->nO);

		for (i=0; i<fc->nI; i++){ /*over all ling. variables which are inputs*/
				v->ihit[i] = (Bool_t *)calloc(sizeof(char), fc->iL[i].n);
				v->imv[i]  = (NumTypeF8_t*)calloc(sizeof(NumTypeF8_t), fc->iL[i].n);
		}
		for (i=0; i<fc->nO; i++){ /*over all ling. variables which are outputs*/
				v->ohit[i] = (Bool_t *)calloc(sizeof(char), fc->oL[i].n);
				v->omv[i]  = (NumTypeF8_t*)calloc(sizeof(NumTypeF8_t), fc->oL[i].n);
		}
		return ;
}
...
void FCF8_calc(const FuzzyControllerF8_t *fc, FCMemF8_t *v, NumTypeF8_t *e, NumTypeF8_t *a)
{
  ...
  for (i=0; i<fc->nR; i++){ /*over all premises*/
	...
	/* membershipvalue is correct (negated sets) */
        index=-index;
        mv=opF8Sub(F8One,v->imv[j][index]);                   /*set was negated so correct membershipvalue */
        v->ihit[j][index]=opF8G(mv,0);                    /*and also correct the hit flag*/
      }
      else{
    ...
    if (hit){
      res=opF8Mul(res, fc->w[i]);   /*multiply weightness*/
      for (j=0; j<fc->nO; j++){
        if (!fc->con[i*fc->nO+j]) continue;   /*no conclusion set*/
        index=fc->con[i*fc->nO+j]-1;
        if ((!v->ohit[j][index])||opF8L(v->omv[j][index], res)){     /*set was not hit by another rule or result membershipvalue higher*/
          v->omv[j][index]=res;
          v->ohit[j][index]=1;
        }
      }
